...
namespace ORS_RCM.WebForms.Item
{
    ...
    public class Export_CSV3
    {
        ...
        //public DataTable ChangeTemplate23(DataTable dt, int shopID, string columnName)
        //{ 
        ...
        //                for (int t = 0; t < templateID.Length; t++ )
        //                {
        //                    int start,end ;
        ...
        //                    if (templateID[t].ToString().Contains("IF setValue="))
        //                    {
        //                        start = html.IndexOf("[["+templateID[t].ToString()+"]]");
        //                        end = html.IndexOf("[[/IF]]",start);
        //                        if(end != -1)
        //                        {
        //                           line = html.Substring(start, end - start) + "[[/IF]]";
        //                        }
        //                        word = (templateID[t].ToString().Replace("IF setValue=","")).Trim();
        //                        switch(word)
        //                        {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    while (Regex.IsMatch(html, @"\[\[([^]]*)\]\]"))
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(itemcode) && !string.IsNullOrWhiteSpace(html) && (html.Contains("基本情報") || html.Contains("詳細情報")))
                        {
                            ...
                            if (dtvalue != null && dtvalue.Rows.Count > 0)
                            {
                                ...
                                else if (html.Contains("詳細情報"))
                                {
                                    ...
                                    //for (int k = 4; k >= 1; k--)
                                    //{
                                    //    html = html.SafeReplace("詳細情報" + k, dtTemplateDetail.Rows[0]["Detail_Template" + k].ToString(), true).SafeReplace("詳細情報内容" + k, dtTemplateDetail.Rows[0]["Detail_Template_Content" + k].ToString(), true);
                                    //    if (html.Contains("[[IF setVal=]]"))  // No Need I think. This case is not exist
                                    //    {
                                    //        start = html.IndexOf("[[IF setVal=]]");
                                    //        end = html.IndexOf("[[/IF]]", start);
                                    //        if (end != -1) // If not contain {{/IF}}
                                    //        {
                                    //            line = html.Substring(start, end - start) + "[[/IF]]";
                                    //            html = html.Remove(start, line.Length);
                                    //        }
                                    //        else
                                    //        {
                                    //            html = html.Remove(start, ("[[IF setVal=]]").Length);
                                    //        }
                                    //    }
                                    //    else if (html.Contains("[[IF setVal=詳細情報" + k + "]]"))
                                    //    {
                                    //        start = html.IndexOf("[[IF setVal=詳細情報" + k + "]]");
                                    //        html = html.Remove(start, ("[[IF setVal=詳細情報" + k + "]]").Length);
                                    //    }
                                    //    //else if (!string.IsNullOrWhiteSpace(dtTemplateDetail.Rows[0]["Detail_Template" + k].ToString()) && html.Contains("[[IF setVal=" + dtTemplateDetail.Rows[0]["Detail_Template" + k].ToString()))
                                    //    else if (html.Contains("[[IF setVal=" + dtTemplateDetail.Rows[0]["Detail_Template" + k].ToString()+"]]"))
                                    //    {
                                    //        start = html.IndexOf("[[IF setVal=" + dtTemplateDetail.Rows[0]["Detail_Template" + k].ToString() + "]]");
                                    //        html = html.Remove(start, ("[[IF setVal=" + dtTemplateDetail.Rows[0]["Detail_Template" + k].ToString() + "]]").Length);
                                    //    }
                                    //}
                                    #endregion
                                }
                        ...
                        if (html.Contains("商品名"))
                        {
                            ...
                            else if (columns.Contains("Item_Name")) //For Preview
                            {
    ...
    public static class StringExtensions
    {
        ...
        {
            //if (find.Contains("product.product_id")) 
            //{
            //    string textToFind = matchWholeWord ? string.Format(@"\b{0}\b", find) : " ";
            //    return Regex.Replace(input, textToFind, replace);
            //}
            //else 
            //if (find.Contains("基本情報") || find.Contains("詳細情報")) // {{ }}
            //{
    ...
    #region
                        /*
                            for (int j = 0; j < dtTemplateDescription.Rows.Count; j++)
                            {
                                if (dtTemplateDescription.Rows[j]["Template_ID"].ToString() == Key)
                                {
                                    Value = dtTemplateDescription.Rows[j]["Template_Description"].ToString();
    ...
    #region
    /*
                    if (!string.IsNullOrWhiteSpace(itemcode) && !string.IsNullOrWhiteSpace(html))
                    {
                        Template_Detail_BL template = new Template_Detail_BL();
                        DataTable dtTemplateDetail = template.SelectByItemCode(itemcode);
                        
                            for (int k = 20; k >= 1; k--)
                            {
                                if (dtTemplateDetail != null && dtTemplateDetail.Rows.Count > 0)
                                {
                                    html = html.SafeReplace("基本情報" + k, dtTemplateDetail.Rows[0]["Template" + k].ToString(), true).SafeReplace("基本情報内容" + k, dtTemplateDetail.Rows[0]["Template_Content" + k].ToString(), true);
                                }
                                else
                                {
                                    html = html.SafeReplace("基本情報" + k, "", true).SafeReplace("基本情報内容" + k, "", true);
                                ...
                                if (html.Contains("{{IF setVal=}}"))  //Value is null . Remove a line.
                                {
                                    int start = html.IndexOf("{{IF setVal=}}");
                                    int end = html.IndexOf("{{/IF}}", start);
                                    string result = html.Substring(start, end - start) + "{{/IF}}";
                                    html = html.Remove(start, result.Length);
                                }
                                else
                                {
                                    int start;
                                    if (dtTemplateDetail != null && dtTemplateDetail.Rows.Count>0)
                                    {
                                         start = html.IndexOf("{{IF setVal=" + dtTemplateDetail.Rows[0]["Template" + k].ToString() + "}}");
                                    }
                                    else
                                    {
                                         start = -1;
                                    }
                                    if (start != -1) // Value is not null. Remove {{IF ....}} {{/IF}}
                                    {
                                        html = html.Remove(start, ("{{IF setVal=" + dtTemplateDetail.Rows[0]["Template" + k].ToString() + "}}").Length);
                                    }
                                }
                            }
                            //html = html.Replace("{{/IF}}", "").Replace("{", "").Replace("}", "");
                        }*/
    #endregion
...
//                DataTable dtItem = dtvalue.Select("SN=" + t + "").CopyToDataTable();
//                replaceword = "";
//                if (html.Contains("[[IF setVal=画像" + t + "]]"))
//                {
//                    if (!string.IsNullOrWhiteSpace(replaceword))
//                    {
//                        html = html.SafeReplace("画像" + t, replaceword, true);
//                        html = html.Remove(start, ("[[IF setVal=画像" + t + "]]").Length);
//                    }
//                    else
//                    {
//                        #region Delete Line
...
//                if (html.Contains("[[IF setVal=画像" + t + "]]"))
//                {
//                    start = html.IndexOf("[[IF setVal=画像" + t + "]]");
//                    end = html.IndexOf("[[/IF]]", start);
//                    if (end != -1)
//                    {
//                        line = html.Substring(start, end - start) + "[[/IF]]";
...
//                else
//                {
//                    html = html.SafeReplace("画像" + t, "", true);
//                }
//            }
//        }
//    }
