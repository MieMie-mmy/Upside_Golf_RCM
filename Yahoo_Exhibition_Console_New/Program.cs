...
namespace Yahoo_Exhibition_Console_New
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                Console.Title = "Yahoo_Exhibition_Console_New";
                ...
                if (dtShopList != null && dtShopList.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtShopList.Rows)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(list))
                        {
                            ...
                            export.YahooFilterSKU(dtImage, dtSelect, dtItem, dtCategory, int.Parse(dr["ID"].ToString()));
                        }
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                cmd.Parameters.AddWithValue("@ErrorDetail", "Yahoo_Exhibition_Console_New " + ex.ToString());
                cmd.Connection.Open();
                cmd.ExecuteNonQuery();
        ...
        static DataTable GetShopList()
        {
            try
            {
                ...
                string quary = "SELECT ID,Mall_ID FROM Shop WHERE Mall_ID = 2";
        ...
        static DataTable SelectLogExhibitionItem(int shop_id, int mall_id,string list)
        {
            try
            {
                DataTable dt = new DataTable();
                SqlConnection connectionstring = new SqlConnection(connectionString);
                SqlDataAdapter sda = new SqlDataAdapter("SP_Log_Exhibition_Item_SelectByShop_Yahoo", connectionstring);
        ...
        static DataTable SelectLogExhibitionSelect(int shop_id, int mall_id,string list)
        {
            try
            {
                DataTable dt = new DataTable();
                SqlConnection connectionstring = new SqlConnection(connectionString);
                SqlDataAdapter sda = new SqlDataAdapter("SP_Log_Exhibition_Select_SelectByShop_Yahoo", connectionstring);
                sda.SelectCommand.CommandType = CommandType.StoredProcedure;
                sda.SelectCommand.Parameters.AddWithValue("@Shop_ID", shop_id);
                sda.SelectCommand.Parameters.AddWithValue("@Mall_ID", mall_id);
                sda.SelectCommand.Parameters.AddWithValue("@Exhibit_ID", list);
